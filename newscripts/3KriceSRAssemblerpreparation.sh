...
trap '(echo $0; date; hostname) | mailx -s "EARLY EXIT from $0" "twmccart@iu.edu"' EXIT
...
# Run SRAssembler with -P option to pre-process reads
    ...
    mpirun -n "$(($(nproc) - 1 ))" SRAssembler_MPI -p $SRAconfigurationfile -l $library -r $readslocation -o ./$cultivar -P -q $library -t cdna && {
    ...
    (date; hostname) | mailx -s "$cultivar reads are processed" "twmccart@iu.edu"
    }
done

(echo $0; date; hostname) | mailx -s "$0 run is complete" "twmccart@iu.edu"
